...
# Predict on test.csv
test_df <- read.csv("src/data/test.csv")

# Apply same preprocessing
test_df$Age[is.na(test_df$Age)] <- mean(df$Age, na.rm = TRUE)
test_df$Fare[is.na(test_df$Fare)] <- mean(df$Fare, na.rm = TRUE)
test_df <- test_df %>% drop_na(Embarked)
test_df$Sex <- ifelse(test_df$Sex == "male", 0, 1)
test_df$Embarked <- recode(test_df$Embarked, "C" = 0, "Q" = 1, "S" = 2)
test_df <- test_df %>% select(Pclass, Sex, Age, SibSp, Parch, Fare, Embarked)
